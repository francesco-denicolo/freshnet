...
for imp in IMPUTERS_ORDER:
    for fc in FORECASTERS_ORDER:
        ...
        med = {c: ps[c].dropna().median() for c in ['hourly_wape','hourly_wpe','daily_wape','daily_wpe']}
...
for fc in ['MLP (M5 lags)', 'LGB (M5 lags)', 'DoW Mean']:
...
FC_TO_TEST = ['Global Mean', 'DoW Mean', 'MA (K=21)', 'LGB (M5 lags)', 'MLP (M5 lags)']
...
for ax_idx, (fc, ax) in enumerate(zip(['MLP (M5 lags)', 'LGB (M5 lags)'], axes)):
    ...
    ax.scatter(x_vals, y_vals, s=100, zorder=5, color=COLORS[:len(x_vals)])
