"""
12c_pinn_relaxed.py — PINN-Retail Relaxed (no I*≥D* constraint)
================================================================
Identico al PINN standard (12_pinn.py, variant F con lag M5-style)
ma con il sotto-vincolo I*≥D* (in-stock) RIMOSSO dalla loss.

L_boundary = solo I*≈0 durante stockout (senza I*≥D* durante in-stock).
Obiettivo: verificare se rimuovere questo vincolo riduce il bias (WPE),
dato che il gradiente tendeva a ridurre D* per soddisfarlo.

Eseguire con: freshnet/bin/python notebooks/12c_pinn_relaxed.py
"""
...
# ===========================================================================
print('=' * 72)
print('  PINN-RETAIL RELAXED: NO I*>=D* CONSTRAINT')
print('=' * 72)
...
def compute_pinn_loss(D_star, I_star, targets, stock,
    ...
    # --- L_boundary (RELAXED: only stockout -> I* ≈ 0, NO I*>=D* constraint) ---
    n_so = so_mask.sum()
    if n_so > 0:
        i_so = I_star[so_mask]
        V_b = i_so.mean()          # linear violation
        Q_b = i_so.pow(2).mean()   # quadratic violation
    else:
        V_b = torch.tensor(0.0, device=D_star.device)
        Q_b = torch.tensor(0.0, device=D_star.device)
...
# 12. Train PINN
# ---------------------------------------------------------------------------
print('\n4. Training PINN-Retail (Relaxed — no I*>=D*)...')
t0 = time.time()
...
torch.save(model.state_dict(),
           os.path.join(RESULTS_DIR, 'pinn_relaxed_model.pt'))
...
for split_name, data in [('val', val_data), ('test', test_data)]:
    ...
    out_path = os.path.join(RESULTS_DIR,
                            f'pinn_relaxed_{split_name}_per_series.parquet')
...
# ---------------------------------------------------------------------------
print(format_metrics_table(pooled_results, model_name='PINN-Retail (Relaxed)'))
...
for label, prefix in all_baselines.items():
    ...
    if prefix == 'pinn_relaxed' and 'test' in pooled_results:
...
fig.suptitle('PINN-Retail Relaxed — Distribuzione per-serie', fontsize=14)
...
fig.tight_layout()
fig.savefig(os.path.join(FIG_DIR, 'fig42_pinn_relaxed_per_series_distributions.png'),
            dpi=150, bbox_inches='tight')
plt.close(fig)
print('  Salvata: fig42_pinn_relaxed_per_series_distributions.png')
