"""
12d_twostage_pinn_a.py — 2-Stage PINN-A (Imputation + PINN no lags)
=====================================================================
Stage 1: LGB imputation (stockout hours → D̂, riuso approccio notebook 10/11)
Stage 2: PINN variant A (no lag features), ma:
...
Input Stage 2: embeddings(76) + continuous(7) = 83 dim (no lags).
Valutazione: D* vs S_obs originale (come tutti i modelli).

Eseguire con: freshnet/bin/python notebooks/12d_twostage_pinn_a.py
"""
...
PREFIX = 'twostage_pinn_a'


# ===========================================================================
print('=' * 72)
print('  2-STAGE PINN-A: IMPUTATION + PINN (NO LAGS)')
print('=' * 72)
...
# ===========================================================================
print('\n' + '=' * 72)
print('  STAGE 2: PINN-A ON COMPLETED DATA (NO LAGS)')
print('=' * 72)


# ---------------------------------------------------------------------------
# 5. Build dataset arrays (variant A — NO lags, completed_Y as target)
# ---------------------------------------------------------------------------
def build_dataset_arrays(sdata, completed, split,
                         cont_mean=None, cont_std=None):
    """Build arrays for 2-Stage PINN-A — no lags, completed_Y as target."""
    if split == 'train':
        d_min, d_max = 2, 83
    elif split == 'val':
        d_min, d_max = 84, 90
    ...
    return {
        ...
        'cont': cont_arr,
        'lags': lag_arr,
        'targets': target_completed,    # completed_Y for L_data
        'targets_orig': target_original, # S_obs for evaluation
        'stock': stock_arr,
        'store_ids': sid_arr,
        'product_ids': pid_arr,
        'cont_mean': cont_mean,
...
# 12. Build datasets
# ---------------------------------------------------------------------------
print('\n5. Costruzione dataset (NO lag features, completed_Y target)...')
t0 = time.time()

print('  Building train data...')
train_data = build_dataset_arrays(series_cache, completed_sales, 'train')
...
val_data = build_dataset_arrays(series_cache, completed_sales, 'val',
                                 cont_mean=train_data['cont_mean'],
                                 cont_std=train_data['cont_std'])
print(f'  Val:   {len(val_data["targets"]):,} samples')

print('  Building test data...')
test_data = build_dataset_arrays(series_cache, completed_sales, 'test',
                                  cont_mean=train_data['cont_mean'],
                                  cont_std=train_data['cont_std'])
...
# ---------------------------------------------------------------------------
print('\n7. Valutazione su val e test (D* vs S_obs)...')
...
# 15. Risultati
# ---------------------------------------------------------------------------
print(format_metrics_table(pooled_results, model_name='2-Stage PINN-A'))
...
fig.suptitle('2-Stage PINN-A — Distribuzione per-serie', fontsize=14)
...
fig.tight_layout()
fig.savefig(os.path.join(FIG_DIR, 'fig43_twostage_pinn_a_distributions.png'),
            dpi=150, bbox_inches='tight')
plt.close(fig)
print('  Salvata: fig43_twostage_pinn_a_distributions.png')
