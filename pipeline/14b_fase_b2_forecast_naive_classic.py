"""
14b_fase_b2_forecast_naive_classic.py — Naive forecaster su completed_sales per 3 nuovi imputer
=================================================================================================
9 celle: 3 imputer (forward_fill, seasonal_naive, linear_interp) × 3 naive forecaster.
"""
...
print('=' * 72)
print('  FASE B2 EXTRA — NAIVE su completed_sales (3 nuovi imputer, ore 6-22)')
print('=' * 72)
...
for imp_key, imp_label in IMPUTERS.items():
    ...
    if not os.path.exists(cs_path):
        print(f'  SKIP: missing {cs_path}')
        continue

    df_cs = pd.read_parquet(cs_path)
    cs_sales = np.array(df_cs['hours_sale'].tolist(), dtype=np.float64)
    # Make sure it's sliced
    if cs_sales.shape[1] == 24:
        cs_sales = cs_sales[:, H_START:H_END]
    completed_full = sales_orig.copy()
    cs_keys = (df_cs['store_id'].astype(str)+'_'+df_cs['product_id'].astype(str)+'_'+df_cs['dt'].astype(str)).values
    full_keys = (df_full['store_id'].astype(str)+'_'+df_full['product_id'].astype(str)+'_'+df_full['dt'].astype(str)).values
...
print('  RIEPILOGO')
print('=' * 72)
print(f'\n  {"Imputer":<18} {"Forecaster":<16} {"WAPE_h pool":>12} {"WPE_h pool":>11} {"WAPE_h med":>11}')
print('  ' + '-' * 72)
for ck, r in all_results.items():
    p,m=r['pooled'],r['median']
    print(f'  {r["imputer"]:<18} {r["forecaster"]:<16} {p["hourly_wape"]:>12.4f} {p["hourly_wpe"]:>11.4f} {m["hourly_wape"]:>11.4f}')

print('\n' + '=' * 72)
print('  DONE — 14b_fase_b2_forecast_naive_classic.py')
print('=' * 72)
