...
out_path=os.path.join(RESULTS_DIR,f'mase_{cell_key}.parquet')
...
if os.getenv('SMOKE')=='1':
    _sp=df_full[['store_id','product_id']].drop_duplicates().head(500)
    df_full=df_full.merge(_sp,on=['store_id','product_id']).reset_index(drop=True)
    print(f'  SMOKE: trimmed to {len(_sp)} series')
...
# MASE/RMSSE (daily in-stock) per series, using precomputed in-sample scale
dft['ins']=(dft['stock'].values==0).astype(float)
dft['pin']=dft['pred']*dft['ins']; dft['oin']=dft['obs']*dft['ins']
g=dft.groupby(['sid','pid','day_idx'],sort=False)[['pin','oin']].sum().reset_index()
g['ad']=np.abs(g.pin-g.oin); g['sd']=(g.pin-g.oin)**2
per=g.groupby(['sid','pid'],sort=False).agg(ae=('ad','mean'),se=('sd','mean')).reset_index()
per=per.rename(columns={'sid':'store_id','pid':'product_id'})
sc=pd.read_parquet(f'{RESULTS_DIR}/mase_scale.parquet'); per=per.merge(sc,on=['store_id','product_id'])
per['mase']=np.where(per.scale_mae>0,per.ae/per.scale_mae,np.nan)
per['rmsse']=np.where(per.scale_mse>0,np.sqrt(per.se/per.scale_mse),np.nan)
per[['store_id','product_id','mase','rmsse']].to_parquet(out_path,index=False)
print(f'  MASE saved: {out_path} | MASE_med={per.mase.median():.4f} RMSSE_med={per.rmsse.median():.4f}')
sys.exit(0)
recs=[]
