...
TEST_DAY=None  # per-day during recursive inference
...
if os.getenv('SMOKE'):
    sc={k:sc[k] for k in list(sc)[:200]}; MAX_EPOCHS=2; print('[SMOKE]',len(sc),'series')
...
NORM=(tr['cont_mean'],tr['cont_std'],tr['lag_mean'],tr['lag_std'])
va=build_ds('val',*NORM); print(f'  Val: {len(va["targets"]):,}')
# (recursive: test built per-day below; series cache kept)
...
# Recursive test inference (days 91-97)
print('\n4. Recursive test inference (days 91-97)...')
model.eval()
import collections
rec=collections.defaultdict(list)
for d in range(91,98):
    globals()['TEST_DAY']=d
    ted=build_ds('test',*NORM)
    tc=torch.from_numpy(ted['cat']).to(DEVICE); tco=torch.from_numpy(ted['cont']).to(DEVICE); tlg=torch.from_numpy(ted['lags']).to(DEVICE)
    with torch.no_grad():
        ap=[]
        for s in range(0,len(tc),10000):
            e=min(s+10000,len(tc)); ap.append(model(tc[s:e],tco[s:e],tlg[s:e]).cpu().numpy())
    pr=np.concatenate(ap) if ap else np.zeros((0,N_HOURS),dtype=np.float32)
    sid_d=ted['store_ids']; pid_d=ted['product_ids']; y=ted['targets']; st=ted['stock']
    for i in range(len(pr)):
        k=(sid_d[i],pid_d[i]); sd=sc.get(k)
        if sd is not None:
            pos=np.where(sd['days']==d)[0]
            if len(pos): sd['sales_c'][pos[0]]=pr[i]
        rec[k].append((pr[i],y[i],st[i]))
    print(f'    day {d}: {len(pr)} series')
recs=[]
for (sid,pid),lst in rec.items():
    P=np.stack([a[0] for a in lst]); Y=np.stack([a[1] for a in lst]); S=np.stack([a[2] for a in lst])
...
print(f'  Recursive WAPE_h med={ps["hourly_wape"].dropna().median():.4f} ({len(ps)} series) -> {out_path}')
print('DONE')
