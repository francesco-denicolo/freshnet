"""
27_fase_b1_imputation_itransformer.py — iTransformer imputer (ore 6-22)
========================================================================
Adattato da 16_fase_b1_imputation_saits.py.

iTransformer (Liu et al. ICLR 2024) usa attention sulle variabili invece che sul tempo.
Nel paper FreshRetailNet baseline è uno dei 4 imputer in Table 3 (con TFT come forecaster).
"""
...
# iTransformer hyperparameters (mirror SAITS small per coerenza)
D_MODEL = 32
N_HEADS = 4
N_LAYERS = 2
D_FFN = 64
...
print('=' * 72)
print('  iTransformer (small) imputer (ore 6-22)')
print('=' * 72)
...
# ===========================================================================
print(f'\n3. Training iTransformer (patience={PATIENCE}, max {MAX_EPOCHS} epochs)...')
print(f'  Device: {DEVICE}')

optimizer = Adam(lr=LR, weight_decay=1e-5)
saving_path = os.path.join(RESULTS_DIR, 'pypots_itransformer_val')
...
print(f'  iTransformer (val-tuned): WAPE_recovery={wape:.4f}, WPE_recovery={wpe:.4f}')

ta_df = pd.DataFrame([{'imputer': 'iTransformer', 'wape_recovery': wape, 'wpe_recovery': wpe}])
ta_df.to_parquet(os.path.join(RESULTS_DIR, 'traccia_a_itransformer.parquet'), index=False)
...
out_path = os.path.join(COMPLETED_DIR, 'itransformer.parquet')
