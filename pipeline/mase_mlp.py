...
ins=(te['stock']==0).astype(np.float64)
pday=(preds*ins).sum(1); oday=(te['targets']*ins).sum(1)
ns=len(pday); S7=ns//7
pm=pday.reshape(S7,7); om=oday.reshape(S7,7)
sids=te['store_ids'].reshape(S7,7)[:,0]; pids=te['product_ids'].reshape(S7,7)[:,0]
ae=np.abs(pm-om).mean(1); se=((pm-om)**2).mean(1)
sc=pd.read_parquet(f'{RESULTS_DIR}/mase_scale.parquet')
df=pd.DataFrame({'store_id':sids,'product_id':pids,'ae':ae,'se':se}).merge(sc,on=['store_id','product_id'])
df['mase']=np.where(df.scale_mae>0,df.ae/df.scale_mae,np.nan)
df['rmsse']=np.where(df.scale_mse>0,np.sqrt(df.se/df.scale_mse),np.nan)
df[['store_id','product_id','mase','rmsse']].to_parquet(out_path,index=False)
print(f'  MASE saved: {out_path} | MASE_med={df.mase.median():.4f} RMSSE_med={df.rmsse.median():.4f}')
sys.exit(0)
