...
TEST_DAY=None  # set per-day during recursive inference
...
out_path=os.path.join(RESULTS_DIR,f'recursive_{cell_key}_test_per_series.parquet')
...
if os.getenv('SMOKE'):
    _k=df_full.drop_duplicates(['store_id','product_id']).head(200)[['store_id','product_id']]
    df_full=df_full.merge(_k,on=['store_id','product_id']).sort_values(['store_id','product_id','dt_parsed']).reset_index(drop=True)
    print('  [SMOKE] subset to', df_full[['store_id','product_id']].drop_duplicates().shape[0],'series')
...
def build_lgb_ds(split):
    ...
    else: d0,d1=((TEST_DAY,TEST_DAY) if TEST_DAY is not None else (91,97))
    ...
    for ri in range(nd):
        ...
        ad=d-1 if split=='train' else (83 if split=='val' else d-1)
...
model=lgb.train(LGB_PARAMS,ltr,num_boost_round=(20 if os.getenv('SMOKE') else 500),valid_sets=[lva],valid_names=['val'],
                callbacks=[lgb.early_stopping(30),lgb.log_evaluation(100)])
print(f'  Best iter: {model.best_iteration}')
del Xtr,ytr,ltr,lva,Xva; gc.collect()

print('\n3. Recursive test inference (days 91-97)...')
import collections
rec=collections.defaultdict(list)
for d in range(91,98):
    globals()['TEST_DAY']=d
    Xte,yte,ste,site,pite=build_lgb_ds('test')
    pr=np.clip(model.predict(Xte),0,None)
    n=len(site)//N_HOURS
    pr17=pr.reshape(n,N_HOURS); y17=yte.reshape(n,N_HOURS); s17=ste.reshape(n,N_HOURS)
    sid_d=site.reshape(n,N_HOURS)[:,0]; pid_d=pite.reshape(n,N_HOURS)[:,0]
    for i in range(n):
        k=(sid_d[i],pid_d[i]); sc=series_cache.get(k)
        if sc is not None:
            pos=np.where(sc['days']==d)[0]
            if len(pos): sc['sales'][pos[0]]=pr17[i]
        rec[k].append((pr17[i],y17[i],s17[i]))
    print(f'    day {d}: {n} series')
recs=[]
for (sid,pid),lst in rec.items():
    P=np.concatenate([a[0] for a in lst]); Y=np.concatenate([a[1] for a in lst]); S=np.concatenate([a[2] for a in lst])
    ig=S==0; sao=np.abs(Y[ig]).sum(); sae=np.abs(P[ig]-Y[ig]).sum(); se=(P[ig]-Y[ig]).sum(); so=Y[ig].sum()
    recs.append({'store_id':sid,'product_id':pid,
                 'hourly_wape':sae/sao if sao>0 else np.nan,
                 'hourly_wpe':se/so if so!=0 else np.nan})
ps=pd.DataFrame(recs); ps.to_parquet(out_path,index=False)
med=ps['hourly_wape'].dropna().median()
print(f'  Recursive WAPE_h med={med:.4f}  ({len(ps)} series) -> {out_path}')
print('DONE')
