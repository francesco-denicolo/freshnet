"""
seedrun_mlp_multi.py — seed sweep for MLP-M5 on ONE imputer cell, building the
dataset once and looping over many seeds (each seed = re-init + re-train only).
Usage:  SEEDS="0,1,...,19" [HPO_VARIANT=1] [SMOKE=1] python seedrun_mlp_multi.py <imputer_key>
...
if os.getenv('HPO_VARIANT') == '1':
    ...
    HIDDEN = json.loads(hpo['hidden']); DROPOUT = float(hpo['dropout']); LR = float(hpo['lr'])
    ...
    print(f'[HPO] hidden={HIDDEN} dropout={DROPOUT} lr={LR:.3e} bs={BATCH_SIZE} wd={WEIGHT_DECAY:.2e}')
if SMOKE:
    MAX_EPOCHS = 2; PATIENCE = 1
    print('[SMOKE] MAX_EPOCHS=2')

IMP_KEY = sys.argv[1]
SUFFIX = '_hpo' if os.getenv('HPO_VARIANT') == '1' else ''
cell_key = f'{IMP_KEY}__mlp_m5lags' + SUFFIX
print(f'=== MLP-M5 seed sweep × {IMP_KEY} | seeds={SEEDS} | device={DEVICE} ===')
...
def build_ds(split, cm=None, cs_=None, lm=None, ls=None):
    ...
    for (sid,pid),sd in sc.items():
        ...
        for idx in range(len(days)):
            ...
            ld=clags(scc[am],dows[am],dows[idx],K) if K>0 \
...
class MLP(nn.Module):
    def __init__(s,nc,nl):
        ...
        inp=sum(EMB_DIMS.values())+nc+nl; layers=[]
...
# device tensors built once (val + test), reused every seed
ds = DS2(tr['cat'],tr['cont'],tr['lags'],tr['targets'])
nc, nl = tr['cont'].shape[1], tr['lags'].shape[1]
del tr; gc.collect()
vi=va['stock']==0
...
def eval_and_save(model, out_path):
    ...
    for i in range(len(te['store_ids'])):
        k=(te['store_ids'][i],te['product_ids'][i]); sm.setdefault(k,[]).append(i)
    recs=[]
...
for seed in SEEDS:
    ...
    med=eval_and_save(model, out_path)
    print(f'  seed {seed}: best_ep={be} val_WAPE={bw:.6f} test_med_WAPE={med:.4f}  ({time.time()-ts:.0f}s)')
