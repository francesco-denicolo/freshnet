"""
16_fase_b1_imputation_saits.py — SAITS imputer (small) con early stopping (ore 6-22)
======================================================================================
Configurazione conservativa per evitare OOM su MPS:
  n_steps = 510 (30 giorni × 17 ore, allineato al paper baseline)
  Modello small: d_model=32, n_heads=4, n_layers=2, d_ffn=64
  Batch size = 16

Procedura:
1. Carica dati e costruisce 3 finestre da 30 giorni per serie (150K samples)
2. Train/val split 80/20 per serie
3. Train SAITS con MIT loss interna + early stopping su val MSE
4. Predict per completed_sales
5. Eval MNAR (Traccia A) con NaN aggiuntivi
"""
...
# SAITS hyperparameters (small to fit MPS)
D_MODEL = 32
N_HEADS = 4
N_LAYERS = 2
D_FFN = 64
...
print('=' * 72)
print('  SAITS (small) con early stopping (ore 6-22)')
print('=' * 72)
print(f'  Config: n_steps={N_STEPS}, d_model={D_MODEL}, n_heads={N_HEADS}, '
      f'n_layers={N_LAYERS}, d_ffn={D_FFN}, batch={BATCH_SIZE}')
...
# ===========================================================================
# 3. Train SAITS with early stopping
# ===========================================================================
print(f'\n3. Training SAITS (patience={PATIENCE}, max {MAX_EPOCHS} epochs)...')
print(f'  Device: {DEVICE}')

optimizer = Adam(lr=LR, weight_decay=1e-5)
saving_path = os.path.join(RESULTS_DIR, 'pypots_saits_val')
...
if os.getenv('TEST_MASK') == '1':
    ...
    _ck = sorted(_glob.glob(os.path.join(saving_path, '*', 'SAITS.pypots')))
    assert _ck, f'no saved model under {saving_path}'
    model.load(_ck[-1]); print(f'  [TEST_MASK] loaded {_ck[-1]}')
    _PB = 200   # SAITS full attention is O(steps^2 * heads) per sample -> keep batch small
    ...
    print(f'  [TEST masks] SAITS: WAPE_recovery={w:.4f}, WPE_recovery={wp:.4f}  (n={len(mtt):,})')
    pd.DataFrame([{'imputer': 'SAITS', 'wape_recovery': w, 'wpe_recovery': wp}]).to_parquet(
        os.path.join(RESULTS_DIR, 'traccia_a_saits_test.parquet'), index=False)
    sys.exit(0)
...
PREDICT_BATCH = 5000  # smaller batch for inference too
...
print(f'  SAITS (val-tuned): WAPE_recovery={wape:.4f}, WPE_recovery={wpe:.4f}')
...
print('\n' + '=' * 72)
print('  DONE — SAITS con validation')
print('=' * 72)
