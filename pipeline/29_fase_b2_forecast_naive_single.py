...
print('=' * 72)
print(f'  Naive forecaster × {IMP_LABELS[IMP_KEY]} (ore 6-22)')
print('=' * 72)
...
print(f'\n2. Carico completed_sales: {IMP_KEY}...')
cs_path = os.path.join(COMPLETED_DIR, f'{IMP_KEY}.parquet')
if not os.path.exists(cs_path):
    print(f'  ERROR: missing {cs_path}')
    sys.exit(1)
...
for fc in FORECASTERS:
    ...
    for si, ser in enumerate(series_list):
        if (si+1) % 10000 == 0: print(f'    ... {fc}: {si+1:,}/{len(series_list):,}')
        ...
        if fc == 'Global Mean':
            pred = np.tile(gm(sales_cs, days, 90), (ne, 1))
        elif fc == 'DoW Mean':
            p = dwm(sales_cs, days, dows, 90); pred = np.array([p[d] for d in ed])
        else:
            pred = np.tile(ma(sales_cs, days, 90, MA_K), (ne, 1))
    ...
    out_path = os.path.join(RESULTS_DIR, f'{ck}_test_per_series.parquet')
    ps.to_parquet(out_path, index=False)
    print(f'  {fc}: WAPE_h pool={pm["hourly_wape"]:.4f}, med={mm["hourly_wape"]:.4f}, '
          f'WPE_h={pm["hourly_wpe"]:.4f} → {out_path}')

print('\n' + '=' * 72)
print(f'  DONE — Naive × {IMP_LABELS[IMP_KEY]}')
print('=' * 72)
