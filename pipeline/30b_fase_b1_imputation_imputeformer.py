"""
30b_fase_b1_imputation_imputeformer.py — ImputeFormer imputer (ore 6-22)
========================================================================
Adattato da 27_fase_b1_imputation_itransformer.py.

ImputeFormer (Nie et al. KDD 2024) usa learnable embeddings + temporal attention.
Nel paper FreshRetailNet baseline è uno dei 4 imputer in Table 3.
Slim HPs per accelerare su CPU.
"""
...
DEVICE = 'cpu'  # CPU per stabilità (MPS NaN issues con alcuni modelli PyPOTS)

# ImputeFormer slim HPs
N_LAYERS = 1
D_INPUT_EMBED = 16
...
print('=' * 72)
print('  ImputeFormer (slim) imputer (ore 6-22)')
print('=' * 72)
print(f'  Config: n_steps={N_STEPS}, n_layers={N_LAYERS}, d_input_embed={D_INPUT_EMBED}, '
      f'd_learnable_embed={D_LEARNABLE_EMBED}, n_heads={N_TEMPORAL_HEADS}, batch={BATCH_SIZE}')

# ===========================================================================
# 1. Load and prepare data (identico a SAITS)
# ===========================================================================
print('\n1. Caricamento dati...')
...
# ===========================================================================
# 3. Train iTransformer with early stopping
# ===========================================================================
print(f'\n3. Training ImputeFormer (patience={PATIENCE}, max {MAX_EPOCHS} epochs)...')
print(f'  Device: {DEVICE}')

optimizer = Adam(lr=LR, weight_decay=1e-5)
saving_path = os.path.join(RESULTS_DIR, 'pypots_imputeformer_val')

def make_model(device):
    return ImputeFormer(
        n_steps=N_STEPS, n_features=n_features,
        n_layers=N_LAYERS, d_input_embed=D_INPUT_EMBED,
        d_learnable_embed=D_LEARNABLE_EMBED, d_proj=D_PROJ,
        d_ffn=D_FFN, n_temporal_heads=N_TEMPORAL_HEADS,
        dropout=DROPOUT, ORT_weight=1, MIT_weight=1,
...
if os.getenv('TEST_MASK') == '1':
    ...
    _ck = sorted(_glob.glob(os.path.join(saving_path, '*', 'ImputeFormer.pypots')))
    ...
    print(f'  [TEST masks] ImputeFormer: WAPE_recovery={w:.4f}, WPE_recovery={wp:.4f}  (n={len(mtt):,})')
    pd.DataFrame([{'imputer': 'ImputeFormer', 'wape_recovery': w, 'wpe_recovery': wp}]).to_parquet(
        os.path.join(RESULTS_DIR, 'traccia_a_imputeformer_test.parquet'), index=False)
    sys.exit(0)
...
print(f'  ImputeFormer (val-tuned): WAPE_recovery={wape:.4f}, WPE_recovery={wpe:.4f}')

ta_df = pd.DataFrame([{'imputer': 'ImputeFormer', 'wape_recovery': wape, 'wpe_recovery': wpe}])
ta_df.to_parquet(os.path.join(RESULTS_DIR, 'traccia_a_imputeformer.parquet'), index=False)
...
out_path = os.path.join(COMPLETED_DIR, 'imputeformer.parquet')
...
print('\n' + '=' * 72)
print('  DONE — iTransformer imputer')
print('=' * 72)
