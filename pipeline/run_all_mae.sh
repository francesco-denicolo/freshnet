#!/bin/bash
# Runs all 22 scripts in sequence to re-train forecasters with MAE loss.
# Prints clear progress between each script.
...
run() {
    ...
    if [ $rc -ne 0 ]; then
        echo "ERROR! Aborting sequence."
        exit $rc
    fi
}

# ============================================================
# FASE A: Single-stage forecaster (no imputer)
# ============================================================
run "Fase A LGB (2 variants)"           "PYTHONUNBUFFERED=1 $PYBIN pipeline/02_fase_a_lgb.py"
run "Fase A MLP (2 variants)"           "PYTHONUNBUFFERED=1 $PYBIN pipeline/03_fase_a_mlp.py"
...
# ============================================================
# FASE B2: MLP M5 × 10 imputer (incluso mediana_cond — non era stato fatto sanity)
# ============================================================
...
echo ""
echo "============================================================"
echo "ALL 22 SCRIPTS DONE in $(( (T_END - T0) / 60 )) min"
echo "============================================================"
