"""
30_fase_b1_imputation_csdi.py — CSDI imputer (ore 6-22)
========================================================================
Adattato da 27_fase_b1_imputation_itransformer.py.

CSDI (Tashiro et al. NeurIPS 2021) usa score-based diffusion per imputation.
Nel paper FreshRetailNet baseline è uno dei 4 imputer in Table 3.
Slim HPs (small model) per accelerare su CPU.
"""
...
DEVICE = 'cpu'  # CSDI diffusion model unstable on MPS

# CSDI ultra-slim HPs (diffusion model is naturally slow on CPU)
N_LAYERS = 1
N_HEADS = 2
N_CHANNELS = 8       # 16 → 8
D_TIME_EMB = 8
D_FEATURE_EMB = 4
D_DIFFUSION_EMB = 8  # 16 → 8
N_DIFFUSION_STEPS = 10  # 20 → 10
...
print('=' * 72)
print('  CSDI (slim) imputer (ore 6-22)')
print('=' * 72)
print(f'  Config: n_steps={N_STEPS}, n_layers={N_LAYERS}, n_heads={N_HEADS}, '
      f'n_channels={N_CHANNELS}, diffusion_steps={N_DIFFUSION_STEPS}, batch={BATCH_SIZE}')
...
# ===========================================================================
print(f'\n3. Training CSDI (patience={PATIENCE}, max {MAX_EPOCHS} epochs)...')
print(f'  Device: {DEVICE}')

optimizer = Adam(lr=LR, weight_decay=1e-5)
saving_path = os.path.join(RESULTS_DIR, 'pypots_csdi_val')

def make_model(device):
    return CSDI(
        n_steps=N_STEPS, n_features=n_features,
        n_layers=N_LAYERS, n_heads=N_HEADS, n_channels=N_CHANNELS,
        d_time_embedding=D_TIME_EMB, d_feature_embedding=D_FEATURE_EMB,
        d_diffusion_embedding=D_DIFFUSION_EMB,
...
print(f'  CSDI (val-tuned): WAPE_recovery={wape:.4f}, WPE_recovery={wpe:.4f}')

ta_df = pd.DataFrame([{'imputer': 'CSDI', 'wape_recovery': wape, 'wpe_recovery': wpe}])
ta_df.to_parquet(os.path.join(RESULTS_DIR, 'traccia_a_csdi.parquet'), index=False)
...
out_path = os.path.join(COMPLETED_DIR, 'csdi.parquet')
