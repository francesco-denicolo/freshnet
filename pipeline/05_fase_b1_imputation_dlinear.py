"""
06c_dlinear_val.py — DLinear con early stopping su validation (ore 6-22)
=========================================================================
Split 150K samples: 80% train, 20% val. Early stopping patience=5.
Poi predict su tutti i 150K per completed_sales + eval MNAR.

Eseguire con: PYTHONUNBUFFERED=1 freshnet/bin/python notebooks_622/06c_dlinear_val.py
"""
...
print('=' * 72)
print('  DLinear con early stopping su validation (ore 6-22)')
print('=' * 72)
...
# ===========================================================================
# 2. Train/val split (80/20 by series, not by window)
# ===========================================================================
print('\n2. Train/val split...')
# Split by series to avoid leakage (all 3 windows of a series go together)
...
val_series = set(series_indices[n_train_series:])

# Map series to window indices
train_idx = []
val_idx = []
...
# PyPOTS validation requires X_ori (original values without NaN)
# Build X_ori: same as X but with original sales instead of NaN
sales_origin_flat = sales_origin.reshape(n_samples, N_STEPS, 1)
X_all_ori = X_all.copy()
X_all_ori[:, :, 0:1] = sales_origin_flat  # replace sales feature with originals (no NaN)
...
# ===========================================================================
print(f'\n3. Training DLinear (patience=5, max 30 epochs)...')
print(f'  Device: {DEVICE}')

optimizer = Adam(lr=0.001, weight_decay=1e-5)
saving_path = os.path.join(RESULTS_DIR, 'pypots_dlinear_val')

model = DLinear(
    n_steps=N_STEPS, n_features=n_features,
    moving_avg_window_size=N_HOURS // 2 * 2 + 1,
    individual=False, d_model=64,
    ORT_weight=1, MIT_weight=1,
    batch_size=128, epochs=30,  # more epochs, rely on early stopping
    patience=5, optimizer=optimizer,
...
if os.getenv('TEST_MASK') == '1':
    ...
    _ck = sorted(_glob.glob('/Users/utente/Desktop/FreshNetRetail/notebooks_622/results/pypots_dlinear_val/*/DLinear.pypots'))
    assert _ck, 'no saved DLinear model under notebooks_622/results/pypots_dlinear_val'
    ...
    print(f'  [TEST masks] DLinear: WAPE_recovery={w:.4f}, WPE_recovery={wp:.4f}  (n={len(mtt):,})')
    pd.DataFrame([{'imputer': 'DLinear', 'wape_recovery': w, 'wpe_recovery': wp}]).to_parquet(
        os.path.join(RESULTS_DIR, 'traccia_a_dlinear_test.parquet'), index=False)
    sys.exit(0)
...
# ===========================================================================
# 4. Predict on all data (for completed_sales)
# ===========================================================================
...
print(f'  Mean imputed: {imputed_sales.mean():.4f}, Mean original (non-NaN): {sales_origin[~np.isnan(sales_nan)].mean():.4f}')

# ===========================================================================
# 5. Eval MNAR (predict with additional NaN)
# ===========================================================================
print(f'\n5. Valutazione MNAR...')
...
print(f'  DLinear (val-tuned): WAPE_recovery={wape:.4f}, WPE_recovery={wpe:.4f}')

ta_df = pd.DataFrame([{'imputer': 'DLinear', 'wape_recovery': wape, 'wpe_recovery': wpe}])
ta_df.to_parquet(os.path.join(RESULTS_DIR, 'traccia_a_dlinear.parquet'), index=False)
...
out_path = os.path.join(COMPLETED_DIR, 'dlinear.parquet')
...
print('\n' + '=' * 72)
print('  DONE — DLinear con validation')
print('=' * 72)
