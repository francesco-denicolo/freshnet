...
if os.getenv('TEST_MASK') == '1':
    ...
    _ck = sorted(_glob.glob(os.path.join(saving_path, '*', 'TimesNet.pypots')))
    ...
    print(f'  [TEST masks] TimesNet: WAPE_recovery={w:.4f}, WPE_recovery={wp:.4f}  (n={len(mtt):,})')
    pd.DataFrame([{'imputer': 'TimesNet', 'wape_recovery': w, 'wpe_recovery': wp}]).to_parquet(
        os.path.join(RESULTS_DIR, 'traccia_a_timesnet_test.parquet'), index=False)
    sys.exit(0)
